...
void gauss(double * ptr_old, int nx, int ny, int iter, int src[2], double e)
{
  ...
  printf("Gauss method was implemented for %d iterations.\n", iter);
  ...
  plot_gif (ptr_plot, iter, nx, ny, "Gauss Method");
...
int gauss_iteration(double * ptr, int nx, int ny, int src[2], double e, double prc, int iter_max)
{
  ...
  if (iter == iter_max)
    printf("Couldn't reach the desired accuracy after %d iterations for the Gauss method.\n", iter);
